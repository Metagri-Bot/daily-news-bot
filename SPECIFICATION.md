...
| `state/public-opportunities.json` | 通知履歴（Docker名前付きボリューム `bot-state` で永続化） |
...
5. S・Aランクかつ`PUBLIC_OPPORTUNITY_MIN_SCORE`以上の上位8件のみ、OpenAI（`gpt-4.1-mini`）で要約・応募資格・接続理由・次の一手・注意点を生成し再採点。AIが`relevant=false`と判定したものは投稿しない
6. 正規化URLで重複除外し、新規・更新のみEmbed（1メッセージ最大10件）で投稿。0件なら投稿しない
7. 通知履歴に署名（タイトル＋URL＋締切のハッシュ）と確認日時を記録。400日を超えた履歴は自動削除
