...
- 4点以上から最大5件。企業事例の枠を最大3件確保し、残りは得点順・同点なら公開日時順で選定します。
- URL（追跡パラメータ除去）、正規化タイトル、引用符で囲まれた3語以上の体験名で重複排除。異なる見出し・引用符なしの同一施策は残る場合があります。
- Discord送信成功後だけ state/roblox-news-sent.json に履歴を保存（30日保持）。翻訳失敗・送信失敗は未送信として次回再試行します。履歴破損時は再掲を避けて処理を停止します。送信直後のプロセス停止や履歴保存失敗では再掲の可能性があります。
- 本文未取得時の推測は禁止し、「概要のみ確認」と明記。過去施策のKPIを今回の成果に転用しません。Google Newsリンクは本文スクレイピングをせずRSS概要を使用します。

読み取り専用の実取得確認: `node scripts/check-roblox-news.js`。Discord投稿・OpenAI呼び出し・履歴更新は行いません。
