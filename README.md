...
| **農業AI通信** | 月・水・金 9:00 | AI Guide記事を要約してDiscordへ投稿し、Google Sheetsへ記録 |
...
| **日誌素案** | 毎週金曜 6:00 | 前日ぶんの投稿を生ログ形式のまま「日誌素案」チャンネルへ投稿（2,000字超は自動分割） |
...
sequenceDiagram
    ...
    Scheduler->>AIGuideTask: 月・水・金9:00に実行命令
...
⚠ **正は `index.js` の `cron.schedule(...)` です。** 下表は2026-09-24時点の実装に同期しています。
...
| `DIARY_DRAFT_CRON` | `0 6 * * 5` | 毎週金曜6:00 JST（日次のチャンネル投稿ログ5:10のあと） |
