...
| **Robloxビジネス速報** | 毎日 7:00 | 7日以内のRoblox関連英語ニュース・企業事例をAI翻訳・要約 |
...
| **農家・食品パートナー案件レーダー** | 平日 8:20 | Skill Shift・ふるさと兼業の受付可能な案件を評価し、新規・重要更新のみ最大5件通知 |
| **活動ログの自動記録** | リアルタイム | スレッド内の発言をGoogleスプレッドシートに自動記録 |
| **チャンネル投稿ログ** | 毎日 5:10 | 指定した8チャンネル直下の投稿（Bot含む）を専用スプレッドシートへ日次で記録 |
| **日誌素案** | 毎週木曜 6:00 | 前日ぶんの投稿を生ログ形式のまま「日誌素案」チャンネルへ投稿（2,000字超は自動分割） |
...
## Discordチャンネル投稿ログ（日次バッチ）

指定したチャンネル**直下**の投稿を1日1回まとめて、**専用の**Googleスプレッドシートへ記録します。
既存の「活動ログの自動記録」（ニューススレッド内の発言を `User` シートへ記録）とは
別モジュール・別GAS・別スプレッドシートです。

**セットアップ手順の正は [`CHANNEL_LOG_SETUP.md`](CHANNEL_LOG_SETUP.md)。**

| ファイル | 役割 |
|----------|------|
| `discord-channel-log.js` | 収集本体。カーソル以降のメッセージをページングして9列の行に落とす |
| `discord-channel-log-store.js` | 専用GASとのやり取り（カーソル取得・行のupsert）とローカル写しの管理 |
| `ChannelLogCode.gs` | 記録先スプレッドシートに貼るGAS。シート作成・書式固定・重複排除 |
| `scripts/run-discord-channel-log-once.js` | 手動で1回実行（`--dry-run` / `--from-scratch`） |
| `test/discord-channel-log.test.js` | ページング・重複・Bot判定・ID破壊防止のテスト |

### 設計上の決定（2026-09-18 本人判断）

- 収集は**定期バッチのみ**。`messageCreate` によるリアルタイム追記はしない
- 収集範囲は**チャンネル直下のみ**。スレッド内の発言は対象外
- **Botの投稿も記録する**（「◯月◯日のまとめ」のような自動投稿も資産として残す）
- 書き込み先は**新しいGAS＋新しいスプレッドシート**。既存GAS（`.gascode`）には触らない

### 取りこぼしと重複が同時に起きない理由

取得の起点は日付ウィンドウではなく **「前回記録した最後の Message ID」** です。
Discordのsnowflakeは時刻の昇順なので、これをカーソルにすると

- 実行が1日飛んでも、その間の投稿は次回にまとめて入る（取りこぼさない）
- 同じ実行を2回走らせても2回目は0件になる（重複しない）

が同時に成立します。カーソルの**正はスプレッドシート**で、`state/discord-channel-log.json` は写しです。
書き込みが途中で失敗したときはカーソルを進めないので、次回に同じ範囲を取り直します。

🔴 **シートのID列はテキスト書式で固定しています。**
snowflakeは19桁の数値文字列なので、書式を当てないとスプレッドシートが数値と解釈して
`1.40934E+18` のように丸め、IDが復元不能になります。シートは必ず `getLogSheet_()` 経由で作ってください。

---

## 日誌素案（週次バッチ / 毎週木曜）

前日ぶんのDiscord投稿を、`01_input/discord-log_YYYY-MM-DD.md` と**同じ生ログ形式のまま**
「日誌素案」チャンネルへ投稿します。日誌を書く人が、ローカルの書き出しを待たずに
その場で素材を読めるようにするための仕組みです。
...
| 変数 | 既定 | 意味 |
|------|------|------|
| `DIARY_DRAFT_CHANNEL_ID` | （空） | 投稿先。**空のときはジョブ自体を登録しない** |
| `DIARY_DRAFT_CRON` | `0 6 * * 4` | 毎週木曜6:00 JST（日次のチャンネル投稿ログ5:10のあと） |
| `DIARY_DRAFT_OFFSET_DAYS` | `1` | 何日前を対象にするか（1＝前日。木曜に走ると水曜ぶん） |
...
- **要約も見解も入れない**。素材をそのまま渡し、何を書くかは書く人が決める
  （ここで要約すると「素案」でなく「下書き」になり、書く人の判断を先に奪う）
- 🔴 **メンションはすべて無効化して送る**（`allowedMentions: { parse: [] }`）。
  生ログには `<@&ロールID>` がそのまま入っているので、これが無いと**素案の投稿で本番のロールが鳴る**
- 2,000字を超える日は**行の切れ目で分割**し、2通以上になるときだけ `(1/3)` を付ける。
  引用ブロックの途中では切らない（1行が2,000字を超えるときだけ機械的に割る）
- 取得できないチャンネルが1本あっても**投稿は止めない**。サマリに `(取得失敗)` と出したうえで
  監視Webhookへ通知する（権限漏れ1本で素材がまるごと消えるほうが損）
- 対象日は「JSTの日付」を出してから引き算する。UTCで引くと**深夜0時台に1日ずれる**
