...
const DEBUG_LOG_SHEET_NAME = 'GAS受信ログ';
  ...
  try {
    ...
    if (rawBody) {
      try {
        ...
        logReceiveDebug_(requestId, 'json_parse_failed', {
          message: err.toString(),
          rawPreview: rawBody.substring(0, 500)
        });
        data = e.parameter || {};
      }
    } else {
    ...
    logReceiveDebug_(requestId, 'received', {
      type: data.type || '',
    ...
    if (data.type === 'aiGuide') {
      return handleAiGuide(data, requestId);
    }
    ...
    logReceiveDebug_(requestId, 'unknown_request_type', { type: data.type || '', keys: Object.keys(data).join(',') });
    return createJsonResponse_({ status: 'error', requestId: requestId, message: 'Unknown request type' });

  } catch (error) {
    logReceiveDebug_(requestId, 'error', {
      message: error.toString(),
      stack: error.stack || ''
    });
    return createJsonResponse_({ status: 'error', requestId: requestId, message: error.toString() });
  } finally {
...
function logReceiveDebug_(requestId, stage, details) {
  try {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    let sheet = ss.getSheetByName(DEBUG_LOG_SHEET_NAME);
    if (!sheet) {
      sheet = ss.insertSheet(DEBUG_LOG_SHEET_NAME);
      sheet.appendRow(['timestamp', 'request_id', 'stage', 'details_json']);
      sheet.getRange(1, 1, 1, 4).setFontWeight('bold').setBackground('#eeeeee');
    }
    sheet.appendRow([
    ...
    console.error('[GAS receive debug] log failed: ' + logError.toString());
  }
}
 ...
 */
function handleAiGuide(data, requestId) {
  if (!data.title && !data.summary && !data.url) {
    logReceiveDebug_(requestId, 'aiGuide_invalid_payload', { keys: Object.keys(data).join(',') });
    return createJsonResponse_({
      status: 'error',
      requestId: requestId,
      message: 'aiGuide payload is missing title, summary, and url'
    });
  }

  logReceiveDebug_(requestId, 'aiGuide_start', {
    title: data.title || '',
    url: data.url || '',
  ...
  logReceiveDebug_(requestId, 'aiGuide_written', {
    sheetName: sheet.getName(),
    sheetId: sheet.getSheetId(),
    a1Length: articleInfo.length,
    a2Url: cleanUrl
  });
